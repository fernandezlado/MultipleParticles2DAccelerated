...
CONTAINS
  ...
  SUBROUTINE initForwardMap ( phy, geo, alg )
    ...
    CALL createFFT_2D ( fft_ver, 2 * N_y, 2 * N_x )
    

    ALLOCATE ( mon_ver_ker_fft ( 2 * N_y, 2 * N_x ) )
    
    ALLOCATE ( dip_ver_ker_fft ( 2 * N_y, 2 * N_x ) ) 
    ...
    ALLOCATE ( mon_equiv_ver ( 2 * N_y, 2 * N_x ) )

    ALLOCATE ( dip_equiv_ver ( 2 * N_y, 2 * N_x ) )


    ALLOCATE ( field_equiv_ver ( 2 * N_y, 2 * N_x ) )
  ...
  SUBROUTINE computeKernelFFT ( phy, geo, alg, mat, orient_flag, pole_flag, fft_handler)
    ...
    src_x ( 0 * N_x + 1 : 1 * N_x ) = step_x * (/ ( n, n = 0, N_x - 1 ) /)
   
    src_y ( 0 * N_y + 1 : 1 * N_y ) = step_y * (/ ( m, m = 0, N_y - 1 ) /)


    src_x ( 1 * N_x + 1 : 2 * N_x ) = step_x * (/ ( n, n = - N_x, -1 ) /)

    src_y ( 1 * N_y + 1 : 2 * N_y ) = step_y * (/ ( m, m = - N_y, -1 ) /)
    ...
    IF ( pole_flag == 'M' ) THEN
       ...
       ELSE
          ...
          FORALL ( n = 1 : 2 * N_y, m = 1 : 2 * N_x )

             mat(n,m) = G_0 ( phy % k, src_x(m), src_y(n) )
    ...
    ELSE
       ...
       ELSE
          ...
          FORALL ( n = 1 : 2 * N_y, m = 1 : 2 * N_x )
             ...
             mat(n,m) = DG_0 ( phy % k, src_x(m), src_y(n), d_x, d_y )
    ...
    CALL directFFT ( fft_handler, mat )
    ...
    DO n = 1, geo % N_col
       ...
       DO m = 1, geo % N_row
          ...
          IF ( ASSOCIATED ( cellArray ( n, m ) % innerObstacle ) ) THEN
             ...
             CALL StoreEquivalentSourceAmplitude ( x_ver, mon_equiv_ver ( ( 2 * alg % N_src_ver ) * m + 1 : ( 2 * alg % N_src_ver ) * (m+1), n+1 : n+2 ) , alg % N_src_ver, 'V', 'M' )
             
             CALL StoreEquivalentSourceAmplitude ( x_ver, dip_equiv_ver ( ( 2 * alg % N_src_ver ) * m + 1 : ( 2 * alg % N_src_ver ) * (m+1), n+1 : n+2 ) , alg % N_src_ver, 'V', 'D' )
...
!  Convolution of Horizontal Equivalent sources
   ...
   CALL directFFT ( fft_ver, mon_equiv_ver )

   CALL directFFT ( fft_ver, dip_equiv_ver )

   field_equiv_ver = mon_ver_ker_fft * mon_equiv_ver

   field_equiv_ver = field_equiv_ver + dip_ver_ker_fft * dip_equiv_ver

   CALL inverseFFT ( fft_ver, field_equiv_ver )

  write(*,*) field_equiv_ver( 2, 1)
